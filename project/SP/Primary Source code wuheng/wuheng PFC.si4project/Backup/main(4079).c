...
{   
  ...
  word Temp = (gPSU.ADC.Vpeak*45)>>6;   //[davidchchen]20161219 (Vpek*45)/64 ==> Vpeak*0.707 = Vrms;   
    ...
    //if(pin_RELAY_CTRL == RELAY_ON && VpeakCompare > BROWN_HIGH_REF && pin_PFC_ADJUST_PIN == PFC_ADJUST_ON)
	...
	//else if(VpeakCompare < BROWN_LOW_REF || pin_PFC_ADJUST_PIN == PFC_ADJUST_OFF)
	//else if(pin_AC_OK_CTRL == AC_NOTOK)
	else if(((pin_AC_OK_CTRL == AC_NOTOK) && (VpeakCompare < AcNGRef && gPSU.Vrms_BrownIn == 0)) || (gPSU.PyStatus.bits.INPUT_OVP == 1))
	{
		...
		if(IsDelay(COUNTER_BROWN_OUT))  //[davidchchen]20161202 note->30ms
		{
			...
			//if(VpeakCompare < BROWN_LOW_REF) pin_AC_OK_CTRL = AC_NOTOK;
		}
...
{   
	...
	/*Check PFC OK pin, add pin_PFC_CTRL condition*/
	//if(gPSU.ADC.Vdc.FF > PFC_OK_REF  && pin_PFC_ADJUST_PIN == PFC_ADJUST_ON)
